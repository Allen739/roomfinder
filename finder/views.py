...
def search_rooms(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            logger.debug(f"Search: building={building}, day={day}, time={time}, duration={duration}")
            ...
            if end_time > datetime.time(23, 0) or start_time < datetime.time(7, 0):
                errors.append("The selected time and duration are outside allowed hours (7:00 AM–11:00 PM).")
                logger.warning(f"Invalid time range: start={start_time}, end={end_time}")
            else:
                buffer_end_time = (end_datetime + datetime.timedelta(minutes=10)).time()
            ...
            if not all_rooms.exists():
                ...
                # Find booked rooms (not canceled) during the time range + buffer
                ...
                if not free_rooms:
                    ...
                    errors.append(f"No rooms available in {building_label} on {day} at {time} for {duration} minutes.")
            ...
            for field, error_list in form.errors.items():
                if field == "__all__":
                    errors.extend(error_list)  # Non-field errors (e.g., from clean())
                else:
        ...
        if form.is_valid():
            ...
            if rooms_file:
                try:
                    ...
                    if not all(col in df.columns for col in expected_columns):
                        messages.error(request, f"Rooms file must have columns: {expected_columns}")
                    else:
                        ...
                        for _, row in df.iterrows():
                            ...
                            if room_name and building:
                                ...
                                rooms_count += 1
                        messages.success(request, f"Loaded {rooms_count} rooms.")
                except Exception as e:
                    messages.error(request, f"Error processing rooms file: {str(e)}")
            ...
            if timetable_file:
                try:
                    ...
                    expected_columns = ["Course", "Subject", "Day/Time", "Room", "Building", "Lecturer"]
                    if not all(col in df.columns for col in expected_columns):
                        messages.error(request, f"Timetable file must have columns: {expected_columns}")
                    else:
                        ClassSchedule.objects.all().delete()  # Clear for testing
                        valid_days = {choice[0] for choice in ClassSchedule.DAY_CHOICES}
                        day_map = {
                        ...
                        for index, row in df.iterrows():
                            ...
                            if not match:
                                messages.warning(request, f"Row {index}: Skipped due to invalid Day/Time: {day_time}")
                                continue
                            day, start_time_str, end_time_str = match.groups()
                            day = day_map.get(day, None)
                            if day not in valid_days:
                                messages.warning(request, f"Row {index}: Skipped due to invalid day: {day}")
                                continue
                            try:
                                start_time = timezone.datetime.strptime(start_time_str, "%I:%M %p").time() if "AM" in start_time_str or "PM" in start_time_str else timezone.datetime.strptime(start_time_str, "%H:%M").time()
                                end_time = timezone.datetime.strptime(end_time_str, "%I:%M %p").time() if "AM" in end_time_str or "PM" in end_time_str else timezone.datetime.strptime(end_time_str, "%H:%M").time()
                            except ValueError as e:
                                messages.warning(request, f"Row {index}: Skipped due to invalid time format: {day_time} ({str(e)})")
                                continue
                            room_name = str(row["Room"]).strip()
                            building = str(row["Building"]).strip()
                            try:
                                ...
                                messages.warning(request, f"Row {index}: Skipped: Room {room_name} in {building} not found.")
                                continue
                            ...
                            if not course or not subject or not lecturer:
                                messages.warning(request, f"Row {index}: Skipped: Missing course/subject/lecturer: Course={course}, Subject={subject}, Lecturer={lecturer}")
                                continue
                            ...
                            timetable_count += 1
                        messages.success(request, f"Loaded {timetable_count} timetable entries.")
                        return redirect("finder:search_rooms")
                except Exception as e:
                    messages.error(request, f"Error processing timetable file: {str(e)}")
