    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(
                "rooms.xlsx not found in project root."))
            return
        ...
        if not all(col in df.columns for col in expected_columns):
            self.stdout.write(
                self.style.ERROR(
                    f"Excel file must contain columns: {expected_columns}"))
            return
        ...
        for _, row in df.iterrows():
            ...
            if not room_name or not building:
                self.stdout.write(
                    self.style.WARNING(
                        f"Skipping invalid row: Room={room_name}, Building={building}"))
                continue
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Successfully loaded {created_count} rooms into the database."))
