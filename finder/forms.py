    ...
    def clean (self):
        ...
        if time and duration:
            try:
                ...
                # Calculate end time
                start_datetime = datetime.datetime.combine(datetime.date.today(), time)
                ...
                if end_time > datetime.time(23, 0):
                    raise ValidationError("The selected time and duration extend beyond 11:00 PM.")
                # Check if time is too early (before 07:00)
                if time < datetime.time(7, 0):
                    raise ValidationError("The selected time is before 7:00 AM.")
...
class ExcelUploadForm(forms.Form):
    rooms_file = forms.FileField(label="Rooms Excel File (rooms.xlsx)", required=False)
    timetable_file = forms.FileField(label="Timetable Excel File (timetable.xlsx)", required=False)
    ...
    def clean(self):
        ...
        if not rooms_file and not timetable_file:
            raise forms.ValidationError("At least one file (rooms or timetable) must be uploaded.")
        return cleaned_data